...
pub fn play(mut one: Player, mut two: Player) {
    ...
    loop {
        ...
        let mut move_king = false;
        match board.checkmate(&mut one, &mut two) {
            Some((king, possible)) => {
                if possible {
                    if turn == 2 && king == Colour::White {
                        turn = -1;
                    } else if turn == 1 && king == Colour::Black {
                        turn = -2;
                    } else {
                        move_king = true;
        ...
        turn = match turn {
            1 => if execute_round(&mut board, &mut one, &mut two, move_king) {2} else {-1},
            2 => if execute_round(&mut board, &mut two, &mut one, move_king) {1} else {-2},
            -1 | -2 => { turn },
            _ => unreachable!()
        };
...
fn execute_round(board: &mut Board, active: &mut Player, inactive: &mut Player, move_king: bool) -> bool {
    ...
    if move_king {
        loop {
            ...
            let (before, after) = active.get_move(board);
        ...
        let (before, after) = active.get_move(board);
        if !board.is_empty(after) {
            let name = board.get_figure(after).unwrap().name();
            inactive.capture(after, name);
        }
        board.move_figure(before, after);
        active.move_figure(before, after);
        true
